...
def test_method(
    ...
    test_memory: bool = False
):
    """
    Generic inference pipeline driven fully by config + method signatures.
    """
    ...
    # Batch size
    # -----------------------------
    batch_size = compute_batch_size(cfg, model_name) if test_memory else 1
    ...
    equations = test_df["equation"].tolist()[:batch_size]
    solutions = test_df["solution"].tolist()[:batch_size]

    # (optional) type column support
    types = (
        test_df["type"].tolist()[:batch_size]
        if "type" in test_df.columns
        else ["unknown"] * len(equations)
    )
    ...
    if hasattr(method, "fit") and load_flag is False:
        ...
        train_eqs = (
            train_df["equation"].tolist()[:batch_size]
            if train_df is not None else []
        )

        train_sols = (
            train_df["solution"].tolist()[:batch_size]
            if train_df is not None else []
        )
